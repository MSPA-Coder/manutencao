...
| Aviso de chegada | [`vps/aviso-contatos.sh`](vps/aviso-contatos.sh), [`vps/aviso-contatos.service`](vps/aviso-contatos.service), [`vps/aviso-contatos.timer`](vps/aviso-contatos.timer) | A cada 15 minutos roda `manage.py contatos_novos` no contêiner `web` de cada checkout em `~/apps` que traz o comando e alerta quando chegou solicitação nova pelo formulário do portal. A mensagem leva só a contagem, nunca dado pessoal. Guarda o último instante avisado em `~/.local/state/mspa-aviso-contatos/`; a primeira execução só inicializa, e falha não anda o estado. |
| Câmbio do NetWorth | [`vps/atualizar-cambio.sh`](vps/atualizar-cambio.sh), [`vps/atualizar-cambio.service`](vps/atualizar-cambio.service), [`vps/atualizar-cambio.timer`](vps/atualizar-cambio.timer) | Às 07:10 de São Paulo roda `manage.py atualizar_cambio` no contêiner `web` de cada checkout em `~/apps` que traz o comando. O NetWorth recusa taxa com mais de sete dias, então sem esta coleta o total em reais some da tela uma semana depois. O comando só grava dia já fechado. Máquina sem o NetWorth sai com sucesso; NetWorth presente sem contêiner para rodar a coleta é falha, e alerta. |
| Foto do patrimônio | [`vps/registrar-foto.sh`](vps/registrar-foto.sh), [`vps/registrar-foto.service`](vps/registrar-foto.service), [`vps/registrar-foto.timer`](vps/registrar-foto.timer) | Às 07:40 de São Paulo, depois da coleta de câmbio, roda `manage.py registrar_foto` no contêiner `web` de cada checkout em `~/apps` que traz o comando. O comando refaz as fotos dos últimos sete dias fechados, porque um lançamento de ontem pode ser registrado amanhã. Sem esta execução, o histórico do NetWorth para calado na última foto. Data que fica sem foto (fonte fora do ar ou posição sem cotação) é falha, e alerta; máquina sem o NetWorth sai com sucesso. |
