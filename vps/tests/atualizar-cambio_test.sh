#!/usr/bin/env bash
# Testes herméticos do `atualizar-cambio.sh`, sem Docker, sem NetWorth e sem VPS.
#
...
#   - NetWorth presente e contêiner web em execução: a coleta roda no
#     contêiner do projeto certo, e só nele;
#   - NetWorth presente e SEM contêiner: falha — é o caso em que a série
#     envelheceria calada e o total em reais sumiria da tela;
#   - coleta que falha dentro do contêiner: falha também.
#
# `docker` e `logger` são scripts falsos no PATH, e o script exercitado é o
# mesmo arquivo que roda no servidor.
#
# Uso:
#   docker run --rm -v "${PWD}:/repo:ro" bash:5.2 bash /repo/vps/tests/atualizar-cambio_test.sh
...
CAMBIO="$TESTS_DIR/../atualizar-cambio.sh"
TOTAL=0
FAILED=0
CASE_FAILED=0
SUITE_TMP=$(mktemp -d)
...
# Um checkout em ~/apps. Com `networth`, ele traz o comando de câmbio.
checkout() {
    local nome=$1 tipo=${2:-}
    mkdir -p "$CASE_TMP/apps/$nome"
    if [ "$tipo" = networth ]; then
        mkdir -p "$CASE_TMP/apps/$nome/consolidado/management/commands"
        : >"$CASE_TMP/apps/$nome/consolidado/management/commands/atualizar_cambio.py"
    fi
}
...
case "$1" in
    ps)
        ...
        ;;
    exec)
        printf 'USD: 1 nova, 0 já existiam (de 2026-09-16 a 2026-09-16)\n'
        exit "$EXEC_SAIDA"
        ;;
    *) exit 1 ;;
esac
...
assert_saida 'nenhum aplicativo com coleta de câmbio' 'máquina sem NetWorth diz que não havia o que fazer'
end_case 'sem NetWorth na máquina: sucesso, sem coleta'
...
assert_eq 1 "$(execs)" 'a coleta roda uma vez'
grep -Fq 'exec networth-web-1 python manage.py atualizar_cambio' "$CHAMADAS" \
    || fail 'a coleta roda no contêiner web do NetWorth'
grep -Fq 'exec controle-bancario-web-1' "$CHAMADAS" \
    && fail 'a coleta não pode rodar em aplicativo sem o comando'
assert_saida 'networth: USD: 1 nova' 'a saída do comando é registrada'
end_case 'NetWorth em execução: coleta no contêiner web dele'
...
EXEC_SAIDA=1
roda
assert_eq 1 "$CODIGO" 'coleta que falha no contêiner é falha'
assert_saida 'a coleta de câmbio falhou' 'a falha do comando é registrada'
end_case 'coleta que falha dentro do contêiner: falha'
